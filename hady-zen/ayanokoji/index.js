  ...
  Object.keys(options).map(function (key) {
    switch (key) {
      ...
      case 'emitReady':
        ...
        break;
      default:
        console.log(global.Ayanokoji.logo.login + "Opsi tidak dikenal diberikan ke setoptions: " + key);
        break;
    }
  });
}
...
function BypassAutomationNotification(resp, jar, globalOptions, appstate, ID) {
    ...
    try {
        ...
        console.log(global.Ayanokoji.logo.error + hady)
    }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (html.indexOf("/checkpoint/block/?next") > -1) console.log(global.Ayanokoji.logo.login + "Checkpoint terdeteksi, silakan masuk ke akun untuk memverifikasi.");

    const userID = maybeCookie[0].cookieString().split("=")[1].toString();
    console.log(global.Ayanokoji.logo.login + `ID akun ${userID}.`);
